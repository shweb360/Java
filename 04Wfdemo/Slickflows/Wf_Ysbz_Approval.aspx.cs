...
{
    public partial class Wf_Ysbz_Approval : BasePage
    {
        ...
        protected void InitFlowInfo()
        {
            ...
            if (!string.IsNullOrEmpty(AppInstanceID))
            {
                ...
                if (financialEntity != null && financialEntity.ID > 0)
                {
                    ...
                    strUrl = financialEntity.Attachment;
                        ...
                        {                          
                            ...
                            foreach (var role in roles)
                            {
                                ...
                                if (role.ID == LoginRoleID.ToString())
                                {
                                    ...
                                    strUrl = GetUploadFileUrl(ID, (int)WfName.预算编制流程, LoginRoleID, out isUpload);

                                    if (LoginRoleID == (int)WfRole.财务主管)
                                    {                                       
                                        this.Repeater1.DataSource = UploadFileBll.GetListUploadFile(ID, (int)WfName.预算编制流程, (int)WfRole.办公室财务);
                                    ...
                                    else if (LoginRoleID == (int)WfRole.站长)
                                    {
                                        ...
                                        var m = UploadFileBll.GetModel(ID, (int)WfName.预算编制流程, (int)WfRole.财务主管);
                                        strUrl = m != null ? m.FilePath : "";

                                        showUploder.Style["display"] = "none";

                                        btnSendNext.Text = "同意(党委会审核已通过)";
                                        btnSendNext.Style["display"] = "inline";
                                        //站长确认后不用弹框选择结束
                                        //hiddenNextActivityPerformers.Value = "step[bcbe6f95-8f8c-46b4-e999-7b8966e3d9cd]"; //结束节点
                                        hiddenNextActivityPerformers.Value = string.Format("step[{0}]", Wf_Ysbz_ProcessConfig[1].ToString());
                                    }
                ...
                if (!string.IsNullOrEmpty(instanceId))
                {
                    ...
                    try
                    {
                                   ...
                                   !string.IsNullOrWhiteSpace(this.txtFileName.Value))
                        {
                            ...
                            ////修改覆盖
                            //var m = UploadFileBll.GetModelByUserID(int.Parse(instanceId), (int)WfName.预算编制流程, LoginUserID);
                    ...
                    try
                    {
                        ...
                        //financialEntity.DepManagerRemark = DepManagerRemark;
                        //financialEntity.DirectorRemark = DirectorRemark;
                        //financialEntity.DeputyGeneralRemark = DeputyGeneralRemark;
                        //financialEntity.GeneralManagerRemark = GeneralManagerRemark;
        ...
        protected void btnSendbackProcess_Click(object sender, EventArgs e)
        {
            ...
            if (m != null)
            {
                if (LoginRoleID == 19)
                {
                    m.AKsfzrRemark = hiddenSendbackRemark.Value;
                }
                else if (LoginRoleID == 21)
                {
                    m.AKsfgldRemark = hiddenSendbackRemark.Value;
                }
                else if (LoginRoleID == (int)WfRole.站长)
                {
                    m.ZzRemark = hiddenSendbackRemark.Value;
                }
                else if (LoginRoleID == 20)
                {
                    m.BKsfzrRemark = hiddenSendbackRemark.Value;
                }
                else if (LoginRoleID == 22)
                {
                    m.BKsfgldRemark = hiddenSendbackRemark.Value;
                }
        ...
        public string Getpath(string FilePath)
        {
            ...
            {
                path = "<a href='" + FilePath.Replace("~", "") + "'>下载</a>";
            }
            return path;
        }
    }
