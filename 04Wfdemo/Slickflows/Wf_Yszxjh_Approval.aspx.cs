...
{
    public partial class Wf_Yszxjh_Approval : BasePage
    {
                        ...
                        if (activityEntity != null && roles != null && roles.Count > 0)
                        {
                            ...
                            foreach (var role in roles)
                            {
                                ...
                                if (role.ID == LoginRoleID.ToString())
                                {
                                    ...
                                    strUrl = GetUploadFileUrl(ID, (int)WfName.预算执行计划流程, LoginRoleID, out isUpload);

                                    if (LoginRoleID == (int)WfRole.财务主管)
                                    {
                                        this.Repeater1.DataSource = UploadFileBll.GetListUploadFile(ID, (int)WfName.预算执行计划流程, (int)WfRole.办公室财务);
                                    ...
                                    {
                                        m = UploadFileBll.GetModel(ID, (int)WfName.预算执行计划流程, (int)WfRole.财务主管);
                                        strUrl = m != null ? m.FilePath : "";
                                        ...
                                        //站长确认后不用弹框选择结束
                                        // hiddenNextActivityPerformers.Value = "step[fb95f162-752d-4725-9cf9-f80b4d5b7f72]"; //结束节点
                                        hiddenNextActivityPerformers.Value = string.Format("step[{0}]", Wf_Yszxjh_ProcessConfig[1].ToString());
                                    }
                ...
                if (!string.IsNullOrEmpty(instanceId))
                {
                    ...
                    try
                    {
                                   ...
                                   !string.IsNullOrWhiteSpace(this.txtFileName.Value))
                        {
                            ...
                            ////修改覆盖
                            //var m = UploadFileBll.GetModelByUserID(int.Parse(instanceId), (int)WfName.预算执行计划流程, LoginUserID);
