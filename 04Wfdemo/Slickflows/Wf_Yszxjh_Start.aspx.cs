...
{
    public partial class Wf_Yszxjh_Start : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                txtProcessGUID.Value = Wf_Yszxjh_ProcessConfig[0].ToString();
                if (LoginRoleID > 0)
                {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string processGUID = Wf_Yszxjh_ProcessConfig[0].ToString();
